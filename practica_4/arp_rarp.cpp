...
void ARP_RARP::initField(string field, string representation){
  if(field == HARDWARE){
    ...
    int a = binToDec(representation.substr(0, BYTE));
    int b = binToDec(representation.substr(BYTE-1, BYTE));
    string aux = hexToString(a) + hexToString(b);
    ...
    this->data = hexToString(binToDec(representation.substr(0, BYTE)));
    for(int k = BYTE; k<representation.length(); k += BYTE){
      this->data += '-' + hexToString(binToDec(representation.substr(k, BYTE)));
    }
  }
}
